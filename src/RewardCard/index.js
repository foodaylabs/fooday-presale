...
export default function RewardCard() {
    ...
    const level = pfoodToLevel(Number(formatEther(balance)))
    const amountToNextLevel = parseEther(String(calcAmountToNextLevel(Number(formatEther(balance)))))
    ...
    const progress = Number(formatEther(balance.sub(currLevelRequiredAmount))) / Number(formatEther(nextLevelRequiredAmount.sub(currLevelRequiredAmount)))
        ...
        <Card head={<StyledTitle>{t('rewardCard.title')}</StyledTitle>}>
            <StyledContent>
                ...
                {level < 6 && (
                    <>
                        <StyledProgress>
                            <Typography variant="header3">{t('rewardCard.purchasedAmount', { amount: trim(formatEther(balance)) })}</Typography>
                            ...
                            <StyledProgressLevels>
                                <StyledProgressLevel>
                                    ...
                                    <Typography variant="caption" color="#545864">{trim(formatEther(currLevelRequiredAmount))} pFOOD</Typography>
                                </StyledProgressLevel>
                                <StyledProgressLevel>
                                    <Typography variant="header2">Level {level + 1}</Typography>
                                    <Typography variant="caption" color="#545864">{trim(formatEther(nextLevelRequiredAmount))} pFOOD</Typography>
                                </StyledProgressLevel>
                            </StyledProgressLevels>
                        </StyledProgress>
